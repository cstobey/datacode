...
mkTool name desc props reqs = ToolDefinition
  { toolDefinitionName        = name
  , toolDefinitionDescription = desc
  , toolDefinitionInputSchema = InputSchemaDefinitionObject
      { properties = [(n, InputSchemaDefinitionProperty "string" d) | (n, d) <- props]
      , required   = reqs
      }
  , toolDefinitionTitle = Nothing
  }

toolDefs :: IO [ToolDefinition]
toolDefs = pure
...
getArg :: Text -> [(Text, Text)] -> String
getArg k kvs = T.unpack $ maybe "" id (lookup k kvs)

getWords :: Text -> [(Text, Text)] -> [String]
getWords k kvs = map T.unpack . T.words . T.pack $ getArg k kvs

-- ---------------------------------------------------------------------------
-- Tool dispatch

callTool :: ToolName -> [(ArgumentName, ArgumentValue)] -> IO (Either Error Content)
callTool toolName args = case toolName of
  "cabal_build" -> run dir $ ["build"] ++ extraArgs ++ target
  "cabal_test"  -> run dir $ ["test"]  ++ extraArgs ++ target
  "cabal_run"   -> runTimed 30 dir $ ["run"] ++ extraArgs ++ target ++ ["--"] ++ runArgs
...
run :: FilePath -> [String] -> IO (Either Error Content)
run dir cabalArgs = do
  ...
  case result of
    ...
    Right (ExitSuccess, o, _) -> pure $ Right $ ContentText  $ T.pack $ if null o then "(success)" else o
    Right (ExitFailure n, o, e) ->
      pure $ Right $ ContentText $ T.pack $
        "cabal exited " <> show n <> "\n" <> e <> o

runTimed :: Int -> FilePath -> [String] -> IO (Either Error Content)
...
main :: IO ()
main = runMcpServerStdio serverInfo McpServerHandlers
  { prompts   = Nothing
  , resources = Nothing
